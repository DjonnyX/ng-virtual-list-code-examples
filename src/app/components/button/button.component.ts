...
import { Color, GradientColor, GradientColorPositions, RoundedCorner } from '../../types';
...
export class ButtonComponent implements AfterViewInit, OnDestroy {
  ...
  fillColors = input<GradientColor | null>(null);

  fillPositions = input<GradientColorPositions | null>(null);

  rippleColor = input<Color | null>(null);
  ...
  fillGradientColors = signal<GradientColor | null>(this.fillColors());

  shapeRoundCorner = signal<[number, number, number, number] | undefined>(this.roundCorner());

  rippleEffectColor = signal<Color | null>(this.rippleColor());
