...
import { ThemeService, ITheme } from '../../../theming';
import { GradientColor, GradientColorPositions } from '../../../types';
...
export class MenuButtonComponent {
  ...
  fillColors = signal<GradientColor | null>(null);
  ...
  constructor() {
    ...
    effect(() => {
      ...
      if (theme && contentElement) {
        const preset = this._themeService.getPreset(theme.app.header.menuButton);
        if (preset) {
          if (focused) {
