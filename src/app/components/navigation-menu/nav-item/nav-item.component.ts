...
@Component({
  selector: 'x-nav-item',
  imports: [
    CommonModule, RouterModule, StaticClickDirective,
  ],
  templateUrl: './nav-item.component.html',
  styleUrl: './nav-item.component.scss'
})
export class XNavItemComponent {
  select = output<IVirtualListItem<any>>();
  ...
  groupBackgroundColor = signal<Color>(DEFAULT_BACKGROUND_COLOR);

  groupColor = signal<Color>(DEFAULT_BACKGROUND_COLOR);

  groupFill = signal<Color>(DEFAULT_BACKGROUND_COLOR);

  theme: Signal<ITheme | undefined>;

  api = input<NgVirtualListService>();

  data = input<IVirtualListItem<INavItem> | null>();
