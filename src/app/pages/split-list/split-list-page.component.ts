...
for (let i = 0, l = MAX_ITEMS; i < l; i++) {
  const id = i + 1, type = i === 0 || Math.random() > .895 ? 'group-header' : 'item',
    ...
    fullSize = Math.round(Math.pow(Math.random(), 4)) === 1,
    sticky = 1;
  if (isGroup) {
    groupDynamicIndex++;
  }
...
export class SplitListPageComponent {
  ...
  protected _dl3ListContainerRef = viewChild('dl3List', { read: NgVirtualListComponent });
  ...
  private _minDl3Id: Id = this.groupDynamicItemsWithSnap.length > 0 ? this.groupDynamicItemsWithSnap[0].id : 0;
  get minDl3Id() { return this._minDl3Id; };

  private _maxDl3Id: Id = this.groupDynamicItemsWithSnap.length > 0 ? this.groupDynamicItemsWithSnap[this.groupDynamicItemsWithSnap.length - 1].id : 0;
  get maxDl3Id() { return this._maxDl3Id; };

  dl3ItemId: Id = this._minDl3Id;
  ...
  onButtonScrollDL3ToIdClickHandler = (e: Event) => {
    const list = this._dl3ListContainerRef(), id = this.dl3ItemId;
    if (list && id !== null) {
      list.scrollTo(id, () => {
        list.focus(id);
        console.info(`scrollTo finished. id: ${id}`);
